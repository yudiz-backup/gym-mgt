  ...
  return (
    <>
      <div className={'select-container ' + (className || '')} style={{ width: width || 'unset' }}>
        {labelText && <label className='select-container-label' htmlFor={labelText} title={labelText}>{isImportant ? <>{labelText}<span className='select-container-important'>*</span><span className='select-container-info'>{props.info}</span></> : labelText}</label>}
        <Component
          ref={ref}
        ...
        {errorMessage && <p className="select-container-errorMessage">{errorMessage}</p>}
      </div>
    </>
  )
}
